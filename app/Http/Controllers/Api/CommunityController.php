        ...
        $data = $posts->map(function ($chat) use ($baseUrl) {
            return [
                ...
                'creator' => [
                    ...
                    'photo_url' => $chat->creator->photo ? $baseUrl . '/storage/' . $chat->creator->photo : null,
                ],
                'created_at' => $chat->chat_created_at->toIso8601String(),
                'attachment_url' => $chat->attachment_url ? $baseUrl . '/storage/' . $chat->attachment_url : null,
                'replies' => $chat->replies->map(function ($reply) use ($baseUrl) {
                    return [
                        ...
                        'user' => [
                            ...
                            'photo_url' => $reply->user->photo ? $baseUrl . '/storage/' . $reply->user->photo : null,
                        ],
                        'created_at' => $reply->created_at->toIso8601String(),
                        'attachment_url' => $reply->attachment_url ? $baseUrl . '/storage/' . $reply->attachment_url : null,
                    ];
                })->values(),
            ];
        });
